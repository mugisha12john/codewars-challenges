...
function VigenèreCipher(key, abc) {
  this.encode = function (str) {
    ...
    for (let i = 0; i < str.length; i++) {
      let charIndex = abc.indexOf(str[i]);
      if (charIndex === -1) {
        result += str[i];
        continue;
      }

      let keyIndex = abc.indexOf(key[i % key.length]);
      let newIndex = (charIndex + keyIndex) % abc.length;
      result += abc[newIndex];
    }
    return result;
  };
  ...
  this.decode = function (str) {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      let keyIndex = abc.indexOf(key[i % key.length]);
      let newIndex = (charIndex - keyIndex + abc.length) % abc.length;
      result += abc[newIndex];
    }
    return result;
  };
