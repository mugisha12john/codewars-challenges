...
function incrementString(strng) {
  ...
  let stringOnly = strng.match(/[A-Za-z]/gi).join("");
  if (num === null) {
    num = 1;
    return `${stringOnly}${num}`;
  } else {
    let iteration = Number(num.join("")) + 1;
    let currentLen = String(iteration).length;
    if (num.length === 1) {
      return `${stringOnly}${iteration}`;
    } else if (currentLen !== num.length) {
      let missing = num.length - currentLen;
